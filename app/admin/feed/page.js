    ...
    <div className="font-sans grid items-center justify-items-center p-8 pb-20 sm:p-20">
      <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-full">
        <div className="w-full justify-items-center">
          {' '}
          <h2 className="text-2xl font-bold">种子管理 </h2>
        </div>
        <div className="w-full flex flex-row-reverse">
          <Button className="w-32 ml-8">
            <Link href={'/admin/feed/export'}>导出种子</Link>
          </Button>
          <Button className="w-32 ml-8">
            <Link href={'/admin/feed/import'}>导入种子</Link>
          </Button>
          <Button className="w-32">
            <Link href={'/admin/feed/add'}>添加种子</Link>
          </Button>
        </div>
        <FeedSearchForm />
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-128 whitespace-break-spaces break-all">
                    标签/网址
                  </TableHead>
              ...
              <TableBody>
                {feeds.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      <FetchButton id={'' + row.id} />
                    </TableCell>
                    <TableCell>
                      <ResetButton id={'' + row.id} />
                    </TableCell>
                    <TableCell>
                      <Link href={'/admin/feed/' + row.id + '/listpages'}>
                        分页
                      </Link>
                    </TableCell>
                    <TableCell>
                      <Link href={'/admin/feed/' + row.id + '/items'}>
                        条目
                      </Link>
                    </TableCell>
                    <TableCell>
                      <Link href={'/admin/feed/' + row.id + '/edit'}>编辑</Link>
                    </TableCell>
                    <TableCell>
                      <Link href={'/admin/feed/' + row.id + '/delete'}>
                        删除
                      </Link>
                    </TableCell>
                  </TableRow>
            ...
            <PaginationContent>
              {pageItems.map((item) => (
                <PaginationItem key={item.id}>
                  <PaginationLink href={item.url} isActive={item.isActive}>
                    {item.label}
                  </PaginationLink>
                </PaginationItem>
              ))}
