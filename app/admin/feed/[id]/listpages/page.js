    ...
    <div className="font-sans grid items-center justify-items-center p-8 pb-20 sm:p-20">
      <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-full">
        <div className="w-full justify-items-center">
          {' '}
          <h2 className="text-2xl font-bold">分页列表 </h2>
        </div>
        <div className="w-full flex flex-row-reverse">
          <div className="w-32 ml-8">
            <GenerateListPageButton id={id} />{' '}
          </div>
          <Button className="w-32">
            <Link href={'/admin/listpage/add'}>添加分页</Link>
          </Button>
        </div>
              ...
              <TableBody>
                {listpages.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      <ListpageFetchButton id={'' + row.id} />
                    </TableCell>
                    <TableCell>
                      <Link href={'/admin/listpage/' + row.id + '/edit'}>
                        编辑
                      </Link>
                    </TableCell>
                    <TableCell>
                      <Link href={'/admin/listpage/' + row.id + '/delete'}>
                        删除
                      </Link>
                    </TableCell>
                  </TableRow>
