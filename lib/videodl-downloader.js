...
class VideodlDownloader {
  ...
  downloadVideo(url, options, maxTime) { 
    let spawnOptions = ["-i", url];
    spawnOptions.concat(options);
	this.downloaderProcess = child_process.spawn('videodl', spawnOptions, {cwd: this.downloadDir});
...
export default VideodlDownloader;
