...
class FeedService {
  ...
  async scrapeItems(page, feed) {
      ...
      for (let i = 0; i < count; ++i) {
        ...
        for (var j = 0; j < fields.length; j++) {
          ...
          if (name != '' && selector == '') {
            ...
            if (field_value != null) {
              //支持转换
              field_value = await executeTransformer(
                field_value,
                parserField,
                context
              );
          ...
          if (name != '' && selector != '') {
            ...
            //let field_ele = await this.locatorWithCatch(ele, selector);

            let field_ele_count = await field_ele.count();
            //if(field_ele != null){
            //判断元素是否存在
            if (field_ele_count == 1) {
              let field_value = null;
              if (attribute == 'innerText') {
    ...
    let downloadCommand = config?.videoDownload?.command || 'none';
    for (var i = 0; i < itemList.length; i++) {
      ...
      if (!item) {
        ...
        itemTmp.image = itemObj.image || '';
        itemTmp.video = itemObj.video || '';
        itemTmp.author = itemObj.author || '';
        itemTmp.publishTime = itemObj.publishTime || '';
        itemTmp.downloadStatus = downloadCommand === 'none' ? -1 : 0;
  ...
  async scrapePagers(page, feed) {
    ...
    for (var i = 0; i < pagerList.length; i++) {
      ...
      //如果不存在，创建一个新的分页链接
      if (!listpage) {
        listpage = await prisma.listpage.create({
          data: { url: pagerObj.url, feedId: feedId, fetchStatus: 0 },
        });
      }
    }
  }
  ...
  async fetchItem(page, item) {
    try {
      ...
      let disableAutoScrapeContent =
        config?.itemParser?.disableAutoScrapeContent || false;
      ...
      for (var j = 0; j < fields.length; j++) {
        ...
        if (name != '' && selector != '') {
          ...
          //let field_ele = await this.locatorWithCatch(page, selector);
          let field_ele_count = await field_ele.count();
          //if(field_ele != null){
          //判断元素是否存在
          if (field_ele_count == 1) {
      ...
      let command = config?.videoDownload?.command || 'none';
      let relVideoPath = '';
      if (command !== 'none') {
        relVideoPath = await downloadVideo(config, url);
        itemObj['video'] = relVideoPath;
      }
  ...
  async scrapeComments(context) {
    ...
    //let listElements = await this.locatorWithCatch(page, commentSelector);
    //const textContent = await element.textContent();
    //console.log(textContent);
    let count = 0;
    if (listElements != null) {
      count = await listElements.count();
    }
    ...
    for (let i = 0; i < count; ++i) {
      ...
      if (scrapeComment2) {
        itemTmp['subcomments'] = await this.scrapeSubComments(ele, context);
      }
  ...
  //Playwright自带的 ele.locator(selector)，在找不到元素的情况下，会抛出异常，导致程序中段
  //我们希望，找不到的时候，对应元素不设置，即可，这里加了try catch
  //仅限于定位单个子元素的，定位多个元素的不行，
  async locatorWithCatch(ele, selector) {
    try {
      let subEle = ele.locator(selector);
      await subEle.waitFor({ timeout: 50 });
      return subEle;
    } catch (err) {
      console.log('err', err);
      return null;
  ...
  async scrapeSubComments(commentEle, context) {
    ...
    //const textContent = await element.textContent();
    //console.log("listElements2", listElements);
    const count = await listElements.count();
...
*/
    ...
    for (var i = 0; i < num; i++) {
      ...
      if (!listpage) {
        listpage = await prisma.listpage.create({
          data: { url: url, feedId: feed?.id, fetchStatus: 0 },
        });
      }
  ...
  async getRandomProxy() {
    let proxies =
      await prisma.$queryRaw`SELECT * FROM proxy WHERE status=TRUE ORDER BY RAND() LIMIT 1`;
