...
export default async function testFetch() {
  ...
  let videoUrl =
    'https://www.thinkindrupal.com/sites/default/files/videos/zuizaoqipu.mp4';
  ...
  if (pattern.test(value)) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        valueNew =
          baseUrl +
          `/file/download/${year}/${month}/${date}/${fileName}.${fileExt}`;
    ...
    if (pattern.test(imgUrl)) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          let imgUrlNew =
            baseUrl +
            `/file/images/${year}/${month}/${date}/${fileName}.${fileExt}`;
          $(img).attr('src', imgUrlNew);
        }
